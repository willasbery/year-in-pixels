...
def resolve_apple_subject(request: Request, identity_token: str) -> str:
    if getattr(request.app.state, "allow_insecure_apple_auth", False):
        digest = hashlib.sha256(identity_token.encode("utf-8")).hexdigest()[:24]
        return f"apple_dev_{digest}"
...
def auth_apple(request: Request, identity_token: str) -> dict[str, Any]:
    apple_sub = resolve_apple_subject(request, identity_token)
