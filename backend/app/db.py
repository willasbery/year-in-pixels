...
def configure_db(database_url: str) -> None:
    ...
    if not normalized:
        raise RuntimeError("DATABASE_URL is required for PostgreSQL/Neon.")
...
def _connect(database_url: str) -> psycopg.Connection[Any]:
    # Neon pooled endpoints use pgbouncer transaction mode. Disabling prepared
    # statements avoids server-side statement state assumptions.
...
@contextmanager
def get_conn(database_url: str | None = None) -> Iterator[psycopg.Connection[Any]]:
    resolved_url = (database_url or get_database_url()).strip()
    conn = _connect(resolved_url)
    try:
        yield conn
        conn.commit()
    except Exception:
...
def theme_from_row(row: dict[str, Any] | None) -> dict[str, Any]:
    if row is None:
    ...
    avoid_lock_screen_ui = normalize_theme_avoid_lock_screen_ui(
        row.get("avoid_lock_screen_ui"),
        DEFAULT_THEME["avoid_lock_screen_ui"],
    )
    columns = normalize_theme_columns(row.get("columns"), DEFAULT_THEME["columns"])
...
def upsert_theme(conn: psycopg.Connection[Any], user_id: str, theme: dict[str, Any], updated_at: str | None = None) -> None:
...
def get_theme(conn: psycopg.Connection[Any], user_id: str) -> dict[str, Any]:
    row = conn.execute("SELECT * FROM themes WHERE user_id = %s", (user_id,)).fetchone()
    if row is None:
...
def create_user(conn: psycopg.Connection[Any], apple_sub: str) -> str:
    for _ in range(5):
        ...
        existing = conn.execute("SELECT id FROM users WHERE apple_sub = %s", (apple_sub,)).fetchone()
...
def create_session(conn: psycopg.Connection[Any], user_id: str, *, session_ttl_seconds: int) -> tuple[str, str]:
    created_at = now_iso()
...
def prune_expired_sessions(conn: psycopg.Connection[Any]) -> None:
    now = now_iso()
    conn.execute("DELETE FROM sessions WHERE expires_at <= %s", (now,))


def ensure_dev_session(conn: psycopg.Connection[Any], dev_bearer_token: str, *, session_ttl_seconds: int) -> None:
    ...
    if existing_session is not None:
        user_exists = conn.execute("SELECT 1 FROM users WHERE id = %s", (existing_session["user_id"],)).fetchone()
        if user_exists is not None:
            created_at = now_iso()
            created_at_dt = _parse_iso_datetime(created_at) or dt.datetime.now(dt.UTC)
            expires_at = _iso_after_seconds(created_at_dt, session_ttl_seconds)
            conn.execute(
                "UPDATE sessions SET created_at = %s, expires_at = %s WHERE token = %s",
    ...
    user_row = conn.execute("SELECT id FROM users WHERE apple_sub = %s", ("dev-local-user",)).fetchone()
