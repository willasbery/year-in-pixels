...
async def wallpaper(token: str, request: Request) -> Response:
    with get_conn() as conn:
        user_row = conn.execute("SELECT id FROM users WHERE wallpaper_token = ?", (token,)).fetchone()
        if user_row is None:
            raise HTTPException(status_code=404, detail="Wallpaper token not found.")
        user_id = str(user_row["id"])

    cached = get_cached_wallpaper(user_id)
    ...
    set_cached_wallpaper(user_id, png_bytes)
