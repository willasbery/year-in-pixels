...
export default function MoodPicker({
  ...
  const styles = useMemo(
    () => createStyles(mode, palette, insets),
    [mode, palette, insets],
  );
  const [isMounted, setIsMounted] = useState(visible);
  const [selectedLevel, setSelectedLevel] = useState<MoodLevel | null>(
    entry?.level ?? null,
  );
  ...
  useEffect(() => {
    ...
    return () => sub.remove();
  }, [isMounted, onClose, visible]);

  useEffect(() => {
    if (visible) {
      sheetProgress.setValue(1);
      backdropOpacity.setValue(0);
      const sheetAnimation = Animated.timing(sheetProgress, {
        toValue: 0,
        duration: 240,
        easing: Easing.out(Easing.cubic),
        useNativeDriver: true,
      });
      const backdropAnimation = Animated.timing(backdropOpacity, {
        toValue: 1,
        duration: 200,
        easing: Easing.out(Easing.quad),
        useNativeDriver: true,
      });
      Animated.parallel([sheetAnimation, backdropAnimation]).start();
      return () => {
        sheetAnimation.stop();
    ...
    const sheetCloseAnimation = Animated.timing(sheetProgress, {
      toValue: 1,
      duration: 180,
      easing: Easing.in(Easing.quad),
      useNativeDriver: true,
    });
    const backdropCloseAnimation = Animated.timing(backdropOpacity, {
      toValue: 0,
      duration: 150,
      useNativeDriver: true,
    });

    Animated.parallel([sheetCloseAnimation, backdropCloseAnimation]).start(
      ({ finished }) => {
        if (finished) {
          setIsMounted(false);
        }
    ...
    return () => {
      sheetCloseAnimation.stop();
      backdropCloseAnimation.stop();
    };
          ...
          <View style={styles.actions}>
            ...
            <Pressable
              ...
              style={[
                ...
                !selectedLevel ? styles.primaryButtonDisabled : undefined,
              ]}
            >
...
const createStyles = (mode: AppColorMode, palette: AppPalette, insets: Insets) =>
  StyleSheet.create({
    overlayContainer: {
      position: "absolute",
      top: -insets.top,
    ...
    backdrop: {
      ...
      backgroundColor:
        mode === "dark" ? "rgba(0, 0, 0, 0.65)" : "rgba(24, 21, 18, 0.4)",
    },
    sheet: {
