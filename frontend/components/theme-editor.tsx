...
type ThemeEditorProps = {
  theme: ThemeSettings;
  onSetShape: (shape: ThemeShape) => void;
  onSetSpacing: (spacing: ThemeSpacing) => void;
...
function isMoodPresetActive(
  ...
  return moodScale.every(
    (mood) =>
      current[mood.level].toLowerCase() === preset[mood.level].toLowerCase(),
  );
}

const shapeOptions: { value: ThemeShape; label: string }[] = [
  { value: "rounded", label: "Rounded" },
  { value: "square", label: "Square" },
  { value: "rough", label: "Rough" },
];

const spacingOptions: { value: ThemeSpacing; label: string }[] = [
...
function ThemeEditor({
  ...
  return (
    <View style={styles.card}>
        ...
        <View style={styles.controlSection}>
          ...
          <View style={styles.presetGrid}>
            {moodColorPresets.map((preset) => {
              const active = isMoodPresetActive(
                theme.moodColors,
                preset.moodColors,
              );
              return (
                <Pressable
                  ...
                  style={[
                    styles.presetCard,
                    active ? styles.presetCardActive : undefined,
                  ]}
                >
                  <Text
                    style={[
                      styles.presetLabel,
                      active ? styles.presetLabelActive : undefined,
                    ]}
                  >
                    {preset.label}
                  </Text>
                  <View style={styles.presetSwatches}>
                    {moodScale.map((mood) => (
                      <View
                        ...
                        style={[
                          styles.presetSwatch,
                          { backgroundColor: preset.moodColors[mood.level] },
                        ]}
                      />
                    ))}
                  </View>
        ...
        <View style={styles.controlSection}>
          ...
          <View style={styles.badges}>
            {shapeOptions.map((opt) => (
              <Pressable
                key={opt.value}
                onPress={() => onSetShape(opt.value)}
                style={[
                  styles.badge,
                  theme.shape === opt.value ? styles.badgeActive : undefined,
                ]}
              >
                <Text
                  style={[
                    styles.badgeText,
                    theme.shape === opt.value
        ...
        <View style={styles.controlSection}>
          ...
          <View style={styles.badges}>
            {spacingOptions.map((opt) => (
              <Pressable
                key={opt.value}
                onPress={() => onSetSpacing(opt.value)}
                style={[
                  styles.badge,
                  theme.spacing === opt.value ? styles.badgeActive : undefined,
                ]}
              >
                <Text
                  style={[
      ...
      <Pressable onPress={onResetTheme} style={styles.button}>
